...
import { handleConfigCommand } from "./command-handler.config.js";
import {
  consumeLandingWizardMessageInput,
  handleLandingCommand,
  isLandingWizardActive,
} from "./command-handler.landing.js";
import {
  handleTaskCommand,
  handleWorkflowCommand,
} from "./command-handler.orchestration.js";
...
function resolveGatewayAgentHint(agent: string): string | undefined {
...
`.trim(),
    ...
    case "/task": {
      await handleTaskCommand(terminal, args, { writeLine, writeHtml, escapeHtml, writeSection: (node, title, icon) => writeSection(node, title, icon, writeHtml) });
      return;
    }
    ...
    case "/workflow": {
      await handleWorkflowCommand(terminal, args, { writeLine, writeHtml, escapeHtml, writeSection: (node, title, icon) => writeSection(node, title, icon, writeHtml) });
      return;
    }

    case "/config": {
      await handleConfigCommand(terminal, args, { writeLine, writeHtml, escapeHtml, writeSection: (node, title, icon) => writeSection(node, title, icon, writeHtml) });
      return;
    }

    case "/landing": {
      await handleLandingCommand(terminal, args, {
        writeHtml,
        writeSection: (node, title, icon) => writeSection(node, title, icon, writeHtml),
        escapeHtml,
        renderLandingFiles: (node, files) => renderLandingFiles(node, files, writeHtml),
      });
      return;
    }
...
async function handleMessage(terminal: HTMLElement, content: string): Promise<void> {
  if (isLandingWizardActive()) {
    appendUserMessageLine(terminal, content);
    const consumed = await consumeLandingWizardMessageInput(terminal, content, {
      writeHtml,
      writeSection: (node, title, icon) => writeSection(node, title, icon, writeHtml),
      escapeHtml,
      renderLandingFiles: (node, files) => renderLandingFiles(node, files, writeHtml),
  ...
  try {
    await gatewayClient.sendMessage(currentSessionKey, content, {
      idempotencyKey: runId,
      agentHint: resolveGatewayAgentHint(currentAgent),
    });
  } catch (err) {
    terminalState = "error";
