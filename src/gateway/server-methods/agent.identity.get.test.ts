...
describe("gateway agent.identity.get", () => {
  ...
  it("does not return /avatar/<id> when the configured local avatar path is missing", async () => {
  ...
  it("returns /avatar/<id> when the configured local avatar file exists", async () => {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-identity-"));
